...
func (a *Adapter) Name() string    { return "qingbo" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".rank-item, .data-card, .list-item", func(e *colly.HTMLElement) {
        title := strings.TrimSpace(e.ChildText(".title, .name, h3"))
        value := strings.TrimSpace(e.ChildText(".value, .score, .index"))
        if title != "" {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".rank-item, .data-card, .list-item")
        for _, el := range els {
