...
func (a *Adapter) Name() string    { return "xinbang" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".data-card, .rank-item, .ranking-card, .content-item", func(e *colly.HTMLElement) {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".data-card, .rank-item, .ranking-card, .content-item")
        for _, el := range els {
