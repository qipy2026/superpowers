...
func (a *Adapter) Name() string    { return "douyin_kuaishou" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".data-card, .video-card, .creator-item, .data-row", func(e *colly.HTMLElement) {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".data-card, .video-card, .creator-item, .data-row")
        for _, el := range els {
