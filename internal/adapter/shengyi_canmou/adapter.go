...
func (a *Adapter) Name() string    { return "shengyi_canmou" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".data-card, .panel-card, .metric-item, .data-block", func(e *colly.HTMLElement) {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".data-card, .panel-card, .metric-item, .data-block")
        for _, el := range els {
