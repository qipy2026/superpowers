...
func (a *Adapter) Name() string    { return "jiujiu_doushang" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".data-card, .shop-item, .product-card, .data-row", func(e *colly.HTMLElement) {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".data-card, .shop-item, .product-card, .data-row")
        for _, el := range els {
