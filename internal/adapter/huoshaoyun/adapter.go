...
func (a *Adapter) Name() string    { return "huoshaoyun" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
    ...
    c.OnHTML(".data-card, .product-row, .data-item, .list-row", func(e *colly.HTMLElement) {
    ...
    if len(rows) == 0 && a.rodPool != nil {
        ...
        els, _ := rp.Page().Elements(".data-card, .product-row, .data-item, .list-row")
        for _, el := range els {
