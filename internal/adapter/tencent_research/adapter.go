...
func (a *Adapter) Name() string    { return "tencent_research" }
...
func (a *Adapter) Collect(ctx context.Context, task *adapter.Task) ([]adapter.DataRow, error) {
	...
	c.OnHTML(".article-item, .report-item, .post-item", func(e *colly.HTMLElement) {
		title := strings.TrimSpace(e.ChildText("h3, h2, .title, a"))
		date := strings.TrimSpace(e.ChildText(".time, .date"))
